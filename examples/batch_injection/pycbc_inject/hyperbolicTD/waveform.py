...
def _check_lal_pars(p):
    ...
    if p['b'] is not None:
        ...
        lalsimulation.SimInspiralWaveformParamsInsertImpactParameter(lal_pars, p['b']*b_SI)
    if p['ui'] is not None:
        lalsimulation.SimInspiralWaveformParamsInsertInitialEccAnomaly(lal_pars, p['ui'])
    if p['uf'] is not None:
        lalsimulation.SimInspiralWaveformParamsInsertFinalEccAnomaly(lal_pars, p['uf'])
    if p['li'] is not None:
        lalsimulation.SimInspiralWaveformParamsInsertInitialMeanAnomaly(lal_pars, p['li'])
    if p['lf'] is not None:
        lalsimulation.SimInspiralWaveformParamsInsertFinalMeanAnomaly(lal_pars, p['lf'])
    return lal_pars
...
def get_td_waveform(template=None, **kwargs):
    """[Injected] Return the plus and cross polarizations of a time domain waveform.
...
from pycbc.waveform.nltides import nonlinear_tidal_spa
cpu_fd["TaylorF2NL"] = nonlinear_tidal_spa

from pycbc.waveform.premerger import premerger_taylorf2
cpu_fd['PreTaylorF2'] = premerger_taylorf2

from pycbc.waveform.multiband import multiband_fd_waveform
