...
data_reg = [regression(config, h[i]) for i in range(len(config.ifo))]
...
sparse_table_list = select_pixels(config, net, data_w_reg, wdm_list, threshold_list)
# ERROR: Segment fault
```
