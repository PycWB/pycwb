...
def process_job_segment(working_dir: str, config: Config, job_seg: WaveSegment, compress_json: bool = True,
    ...
    if job_seg.bootstrap:
        base_data = [block_bootstrap(base_data[i], job_seg.bootstrap, job_seg.injections[0]) for i in range(len(job_seg.ifos))]
    ...
    for trail_idx in trail_idxs:
        ...
        # data conditioning
        if not job_seg.bootstrap:
            logger.info("Performing data conditioning for original data")
            tf_maps, nRMS_list = data_conditioning(config, data)
            logger.info("Memory usage: %f.2 MB", psutil.Process().memory_info().rss / 1024 / 1024)

        else: 
            logger.info("Performing fake data conditioning for bootstrap samples")
            tf_maps, nRMS_list = data_conditioning(config, data.copy()) 
            for i in range(len(tf_maps)):
                tf_maps[i].data = data[i] 
                nRMS_list = np.ones(len(nRMS_list.data))
            logger.info("Memory usage: %f.2 MB", psutil.Process().memory_info().rss / 1024 / 1024)
...
def block_bootstrap(data: TimeSeries, bootstrap, injection) -> TimeSeries:
    """
