...
from tensorflow.python.keras.layers import Flatten, Reshape, AveragePooling1D, Input, Dense, Conv1D, MaxPooling1D, UpSampling1D
...
def timeseries_processing(data, window):
    ...
    # input : timeseries, window lenght
    # output: windowed timeseries centered around the absolute maximum, normalized between [0,1], offset = 0.5
    ...
    if data_max> data_min:
        ...
        half = data.argmax()
        if ((half< window+1) or (half>maxim_len-window)): #if the maximum is close to the timeseries edge, padding with the overall mean value
...
def autoencoder(dim1):
    ...
    # input: timeseries lenght after preprocessing ( = 2*window )
