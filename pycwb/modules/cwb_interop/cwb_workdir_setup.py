...
Directory layout produced for each sub_job_seg::
    ...
    {cwb_compare_dir}/
    └── job{index}_trial{trial_idx}/          ← cWB working directory
        ├── config/
...
def create_cwb_workdir(
    ...
    cwb_workdir = os.path.abspath(
        os.path.join(
            cwb_compare_dir,
            f"job{sub_job_seg.index}_trial{sub_job_seg.trial_idx}",
        )
    )
...
def _write_user_parameters_c(config, sub_job_seg, config_dir: str,
    ...
    lines = [
        ...
        f"{I}// trial_idx    : {sub_job_seg.trial_idx}",
