...
def rate(data: np.ndarray, livetime: float, binwidth: float = 0.05):
    x_min, x_max = data.min()-0.5, data.max()+0.5
    t_nbins = int((x_max - x_min) / binwidth)

    x = x_min + np.arange(t_nbins) * binwidth
    num = len(data) - np.searchsorted(np.sort(data), x, side='left')
    y = num / livetime
    xerr = np.zeros(t_nbins)
    yerr = np.sqrt(num) / livetime
...
def plot_1d_histogram(x: np.ndarray | list, colors: str | list, label: str = None, ax = None, bins=20, xlim: tuple = None, x_edges=None):
    ...
    valid = np.isfinite(x_tot)
    n_dropped = (~valid).sum()
    if n_dropped:
        print(f"Warning: {n_dropped} ({n_dropped/len(valid)*100:.1f}%) triggers dropped due to non-finite values")
    x_tot = x_tot[valid]
...
def plot_2d_histogram(x: np.ndarray, y: np.ndarray, xbins: int = 100, ybins: int = 100, 
    ...
    h_masked = np.where(h == 0, np.nan, h.astype(float))
    ...
    ax.set_xlim(*xlim) if xlim else ax.set_xlim(x_min, x_max)
    ax.set_ylim(*ylim) if ylim else ax.set_ylim(y_min, y_max)
    # x_pad = 0.1 * (x_max - x_min)
    # y_pad = 0.1 * (y_max - y_min)
    # ax.set_xlim(*xlim) if xlim else ax.set_xlim(x_min - x_pad, x_max + x_pad)
    # ax.set_ylim(*ylim) if ylim else ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ax.grid(ls='dotted')
