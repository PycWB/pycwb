...
def dump_training_settings(
    ...
    output_file : str, optional
        Explicit output path. Defaults to ``ofile_tag + ".cfg"``.
    metadata : dict, optional
        Extra workflow/run metadata to include for review.
    ...
    """
    ofile = output_file or (ofile_tag + ".cfg")
    lines = [
...
def _format_evals_result(XGB_clf: "xgb.XGBClassifier") -> list[str]:
    """Return text lines describing the XGBoost eval history."""
    try:
        evals_result = XGB_clf.evals_result()
    except Exception as exc:
        return ["[evals_result]", f"  # could not read: {exc}", ""]
    if not evals_result:
        return ["[evals_result]", "  # empty", ""]

    lines: list[str] = []
    for dataset, metrics in evals_result.items():
        for metric, values in metrics.items():
            values = list(values)
            lines.append(f"[evals_result.{dataset}.{metric}]")
            lines.append(f"  n = {len(values)}")
            if values:
                lines.append(f"  first = {values[0]!r}")
                lines.append(f"  last = {values[-1]!r}")
                if "auc" in metric.lower():
                    best_idx = max(range(len(values)), key=lambda i: values[i])
                else:
                    best_idx = min(range(len(values)), key=lambda i: values[i])
                lines.append(f"  best_index = {best_idx}")
                lines.append(f"  best_value = {values[best_idx]!r}")
                lines.append(f"  values = {values!r}")
            lines.append("")
    return lines


def dump_training_output(
    ...
    * **total_cover** – total (summed) cover across all splits on that feature.
    ...
    output_file : str, optional
        Explicit output path. Defaults to ``ofile_tag + ".out"``.
    training_summary : dict, optional
        Extra runtime/model summary values to include for review.
    ...
    """
    ofile = output_file or (ofile_tag + ".out")
    ...
    lines.extend(_format_evals_result(XGB_clf))
...
def _dump_config(
    ...
    dump_training_settings(ofile_tag, xgb_params, ML_list, ML_caps, ML_balance, ML_options)


def _dump_output(XGB_clf: "xgb.XGBClassifier", ofile_tag: str) -> None:
    dump_training_output(XGB_clf, ofile_tag)
