...
def test_generator_in_injection(mocker):
    ...
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'strain', 'L1': l1_ts, 'H1': h1_ts})
    ...
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'strain', 'L1': l1_ts, 'H1': h1_ts, 'V1': _make_ts()})
...
def test_generator_in_config(mocker):
    ...
    h1_ts, l1_ts = _make_ts(), _make_ts()
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'strain', 'H1': h1_ts, 'L1': l1_ts})
...
def test_default_generator_warning(mocker):
    ...
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'polarizations', 'hp': hp_ts, 'hc': hc_ts})
    mocker.patch('pycwb.modules.injection.strain.project_to_detector', return_value=[_make_ts()])
...
def test_tuple_return_backward_compat(mocker):
    ...
    hp_ts, hc_ts = _make_ts(), _make_ts()
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: (hp_ts, hc_ts))
    mocker.patch('pycwb.modules.injection.strain.project_to_detector', return_value=[_make_ts(), _make_ts()])
...
def test_strain_type_return(mocker):
    ...
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: mock_strain)
...
def test_polarizations_projection(mocker):
    ...
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'polarizations', 'hp': mock_hp, 'hc': mock_hc})
    project_to_detector = mocker.patch('pycwb.modules.injection.strain.project_to_detector', return_value=['strain'])
...
def test_mismatched_ifos_error(mocker):
    ...
    # for missing ifos
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'strain', 'H1': _make_ts()})
    with pytest.raises(ValueError, match="ifos"):
        generate_strain_from_injection(injection, config, 4096, ['H1', 'L1'])

    # for extra ifos
    mocker.patch('pycwb.modules.injection.strain.import_function', return_value=lambda **kwargs: {'type': 'strain', 'H1': _make_ts(), 'L1': _make_ts(), 'V1': _make_ts()})
    with pytest.raises(ValueError, match="ifos"):
        generate_strain_from_injection(injection, config, 4096, ['H1', 'L1'])
